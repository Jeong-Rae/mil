...
2. `POST /access` 요청을 받을 수 있어야 한다.
3. 요청의 `type`, `id`, `location`을 읽어 CSV에 기록할 수 있어야 한다.
4. 요청에 따라 location별 현재 상태를 메모리에서 갱신할 수 있어야 한다.
5. `GET /status?location=...` 로 특정 location의 현재 상태 목록을 반환할 수 있어야 한다.
6. 서버 재시작 시 CSV를 다시 읽어 현재 상태를 복원할 수 있어야 한다.
7. `GET /logs?day=...` 로 특정 KST 날짜의 전체 로그를 반환할 수 있어야 한다.
